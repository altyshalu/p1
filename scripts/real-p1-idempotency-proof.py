...
def main() -> int:
    ...
    if sum(duplicate_events.values()) <= 0 and int(latest_metrics.get('sheet_duplicate_skipped') or 0) <= 0 and int(latest_metrics.get('outreach_master_duplicate_skipped') or 0) <= 0:
        raise SystemExit(f'idempotency proof failed: no duplicate-skip evidence found; metrics={latest_metrics}; events={duplicate_events}')
    print(json.dumps({'run_id': created['id'], 'latest_metrics': latest_metrics, 'duplicate_events': duplicate_events}, ensure_ascii=False, indent=2))
    return 0
