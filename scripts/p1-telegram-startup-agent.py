...
def load_candidate_pool(config: Config) -> list[dict[str, Any]]:
    ...
    prompt = f"""
Generate 80 fresh Europe/UK AI-native startup candidates for a VC daily review queue.
...
"""
    ...
    return [normalize_startup(item) for item in startups if isinstance(item, dict)]
